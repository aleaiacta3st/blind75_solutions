...
# The ','.join(values) operation is a powerful Python string method 
# that transforms a list of strings into a single string with each 
# element separated by the specified character(s).
# This takes our list of values (like ["1", "2", "null", "3"]) and 
# creates a single string with commas between each value ("1,2,null,3").

# values = data.split(',') is the mirror operation to the join() method 
# we just discussed:

# split(',') takes a single string and breaks it apart at each comma
# join(',') takes a list of strings and combines them with commas