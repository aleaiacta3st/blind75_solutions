...
# ├─ calls maxDepth(B)
# │    ├─ calls maxDepth(D)
# │    │    ├─ calls maxDepth(None) → returns 0
# │    │    ├─ calls maxDepth(None) → returns 0
# │    │    └─ returns 1 + max(0,0) = 1  ← depth of D
# │    ├─ calls maxDepth(E)
# │    │    ├─ calls maxDepth(None) → returns 0
# │    │    ├─ calls maxDepth(None) → returns 0
# │    │    └─ returns 1 + max(0,0) = 1  ← depth of E
# │    └─ returns 1 + max(1,1) = 2        ← depth of B
# ├─ calls maxDepth(C)
# │    ├─ calls maxDepth(None) → returns 0
# │    ├─ calls maxDepth(None) → returns 0
# │    └─ returns 1 + max(0,0) = 1        ← depth of C
# └─ returns 1 + max(2,1) = 3             ← max depth of tree
